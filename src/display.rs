...
use esp_idf_svc::hal::{delay::Delay, gpio::AnyOutputPin, gpio::PinDriver, gpio::Output};
...
pub struct LcdDisplay {
    pins: [Option<PinDriver<'static, AnyOutputPin, Output >>; 12],
    display_func: u8,
    display_mode: u8,
    display_ctrl: u8,
    offsets: [u8; 4],
...
impl LcdDisplay
{
    ...
    ///     .build();
    /// ```
    pub fn new(rs: AnyOutputPin, en: AnyOutputPin, delay: Delay) -> Self {
        Self {
            pins: [
                Some(PinDriver::output(rs).unwrap()),
                Some(PinDriver::output(en).unwrap()),
                None,
                None,
                None,
                None,
    ...
    ///     .build();
    /// ```
    pub fn with_half_bus(mut self, d4: AnyOutputPin, d5: AnyOutputPin, d6: AnyOutputPin, d7: AnyOutputPin) -> Self {
        // set to four-bit bus mode and assign pins
        self.display_func &= !(Mode::EightBits as u8);
        self.pins[D4 as usize] = Some(PinDriver::output(d4).unwrap());
        self.pins[D5 as usize] = Some(PinDriver::output(d5).unwrap());
        self.pins[D6 as usize] = Some(PinDriver::output(d6).unwrap());
        self.pins[D7 as usize] = Some(PinDriver::output(d7).unwrap());
        self
    }
    ...
    pub fn with_full_bus(mut self, d0: AnyOutputPin, d1: AnyOutputPin, d2: AnyOutputPin, d3: AnyOutputPin, d4: AnyOutputPin, d5: AnyOutputPin, d6: AnyOutputPin, d7: AnyOutputPin) -> Self {
        // set to eight-bit bus mode and assign pins
        self.display_func |= Mode::EightBits as u8;
        self.pins[D0 as usize] = Some(PinDriver::output(d0).unwrap());
        self.pins[D1 as usize] = Some(PinDriver::output(d1).unwrap());
        self.pins[D2 as usize] = Some(PinDriver::output(d2).unwrap());
        self.pins[D3 as usize] = Some(PinDriver::output(d3).unwrap());
        self.pins[D4 as usize] = Some(PinDriver::output(d4).unwrap());
        self.pins[D5 as usize] = Some(PinDriver::output(d5).unwrap());
        self.pins[D6 as usize] = Some(PinDriver::output(d6).unwrap());
        self.pins[D7 as usize] = Some(PinDriver::output(d7).unwrap());
    ...
    ///     .build();
    /// ```
    pub fn with_rw(mut self, rw: AnyOutputPin) -> Self {
        self.pins[RW as usize] = Some(PinDriver::output(rw).unwrap());
        self
    }
    ...
    pub fn with_backlight(mut self, backlight_pin: AnyOutputPin) -> Self {
        self.pins[A as usize] = Some(PinDriver::output(backlight_pin).unwrap());
        self
    }
